...
// likening    comparison
// likener     comparator
// quid        statement (programming)
// quidlet     expression (programming)
// neven       to subject
// nevener     predicate (programming)
// soothquid   fact, true saying
// withchoose  reject, disapprove
// asoothe     prove, confirm
// working     function
// worker      invocable, functor
// forebizen   template
// nameroom    namespace
// fremer      operator
// ask         examination
// anward      present, current
// onsetting   initialization
// awending    translation
// awender     compiler (programming)
// awendend    mutable
// hele        encrypt
// heling      encryption
// unhele      decrypt
// unheling    decryption
// wale        switch (programming)
// wrixling    variant
// fay         join together
// eke         addition, add
// bestead     replace
...
// bework      edit
// beworker    editor
// unflagging  indefatigable
// sicker      certain
// gleeman     musician
// inwit       conscience
// agenbite    remorse
// stow        place, location
// stead       place (general, also figurative)
...
// ```
// =<f1>            # unto f1
// *"f2"            # unto anward nameroom
// api,App<"common" # only bring `api`
// <"a"             # bring file contents

// =db{} [](_ x) = (abide api(x)).json() # fremer overloading

// <T,U>= U[] = ?(T x){ for U&:x{} }     # forebizens, asks
// <F,R,Ts>= R(Ts...) = ?R(F f, Ts xs...){ f(xs...) }

// <App>
//   <div> ...<p>`${[]"name" $ db["clients"])}`

// [N][M]<<A>+B> ([N][P]A a)*([P][M]B b) =
//     (<_...K>=>((a[{K}/N,]*b[,{K}%N])+...)...)(<[..N*M]...>)
// ```
// */
...
// // Note: no support for tab, CR, etc.; not needed as modern editors can automatically convert
// these.
...
// {}; struct tmplit : seq<one<'`'>, star<tmplit_seq>, one<'`'>> {}; struct literal : seq<sor<bool_,
// hexfloat, hexint, float_, int_>> {};
...
// void dump_hex(auto &&xs, auto d_f, bool dim = true)
// {
